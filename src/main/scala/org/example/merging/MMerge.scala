...
object MMerge {
  ...
  private def mergeWithArray(array: List[JValue], singleValue: Atomic): JArray = JArray(array :+ singleValue)

  private def mergeFields(vs1: List[JField], vs2: List[JField]): List[JField] = {
    @tailrec def mergeRec(acc: List[JField], xleft: List[JField], yleft: List[JField]): List[JField] = xleft match {
      case Nil => acc ++ yleft
  ...
  private def mergeVals(vs1: List[JValue], vs2: List[JValue]): List[JValue] = {
    @tailrec def mergeRec(acc: List[JValue], xleft: List[JValue], yleft: List[JValue]): List[JValue] = xleft match {
      case Nil => acc ++ yleft
